classdef GTZAN < handle
    ...
    methods
        function obj = GTZAN(pathList, batchSize)
            ...
            obj.index = (1:batchSize) - obj.batchSize;
        end
        ...
        function [gtzan_train, gtzan_valid, gtzan_test] = stratify(obj, N_training, N_validation, N_testing)
            ...
            gtzan_train = GTZAN(stratifiedFilePath{1}, obj.batchSize);
            gtzan_valid = GTZAN(stratifiedFilePath{2}, obj.batchSize);
            gtzan_test = GTZAN(stratifiedFilePath{3}, obj.batchSize);
        end
        ...
        function feature = batchFormat(obj, cellFormat)
            ...
            if sum(sum(sum(isinf(feature),3),2)) > 0 ...
                || sum(sum(sum(isnan(feature),3),2)) > 0 ...
                || sum(sum(sum(find(feature==0),3),2)) > 0
            
                fprintf('check it out\n');
                A = cellfun(@isnan,cellFormat,'UniformOutput',false);
                B = cellfun(@find,A,'UniformOutput',false);
            end
            
            %{
            for i=1:10
                surf(squeeze(feature(:,i,:)),'LineStyle','none');view(0,90);axis tight;
                pause
            end
            %}
        end
